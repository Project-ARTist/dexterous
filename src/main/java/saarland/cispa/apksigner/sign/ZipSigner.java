...
public class ZipSigner {
    ...
    protected String autoDetectKey(String mode, Map<String, ZioEntry> zioEntries)
        ...
        for (Map.Entry<String, ZioEntry> entry : zioEntries.entrySet()) {
            ...
            if (entryName.startsWith("META-INF/") && entryName.endsWith(".RSA")) {
                ...
                if (keyName != null) {
                    Log.d(TAG, String.format("Auto-determined key=%s using md5=%s", keyName, md5String));
                } else {
                    Log.d(TAG, String.format("Auto key determination failed for md5=%s", md5String));
                }
    ...
    private KeySpec decryptPrivateKey(byte[] encryptedPrivateKey, String keyPassword)
        ...
        try {
            ...
            Log.e(TAG, "signapk: Password for private key may be bad.");
            throw ex;
        }
    }
    ...
    private void writeSignatureBlock(KeySet keySet, byte[] signatureFileBytes, OutputStream out)
        ...
        if (keySet.getSigBlockTemplate() != null) {
            ...
            Log.d(TAG, "Signature Decrypted: \n" + HexDumpEncoder.encode(tmpData));
        } else {
            try {
                byte[] sigBlock = null;
                // Use reflection to call the optional generator.
                Class generatorClass = Class.forName(ApkZipSir.SIGNATURE_BLOCK_GENERATOR);
                Method generatorMethod = generatorClass.getMethod(ApkZipSir.SIGNATURE_BLOCK_GENERATOR_METHOD, KeySet.class, (new byte[1]).getClass());
    ...
    public void signZip(URL keystoreURL,
        ...
        try {
            ...
            setKeys(CustomKeySigner.KEY_NAME_CUSTOM, publicKey, privateKey, signatureAlgorithm, null);
     ...
     */
    public void signZip(final String inputZipFilename, final String outputZipFilename)
            throws IOException, GeneralSecurityException {
        ...
        final ZipInput input = ZipInput.read(inputZipFilename);
        FileOutputStream fileOutStream = new FileOutputStream(outputZipFilename);
        signZip(input.getEntries(), fileOutStream, outputZipFilename);
        if (input != null) {
            input.close();
        }
     ...
     * android-sun-jarsign-support.jar must be in the classpath.
     */
    public void signZip(final Map<String, ZioEntry> zioEntries,
                        final OutputStream outputStream,
                        final String outputZipFilename)
            throws IOException, GeneralSecurityException {
        progressHelper.initProgress();
        ...
        if (keySet == null) {
            ...
            // Auto-determine which keys to use
            final String keyName = this.autoDetectKey(keymode, zioEntries);
            if (keyName == null) {
        ...
        try {
            ...
            for (final ZioEntry entry : zioEntries.values()) {
            ...
            if (canceled) return;
            ZioEntry zipFileEntry = new ZioEntry(JarFile.MANIFEST_NAME);
            zipFileEntry.setTime(timestamp);
            manifest.write(zipFileEntry.getOutputStream());
            zipOutput.write(zipFileEntry);

            // CERT.SF
            zipFileEntry = new ZioEntry(CERT_SF_NAME);
            zipFileEntry.setTime(timestamp);
            ...
            zipFileEntry = new ZioEntry(CERT_RSA_NAME);
            zipFileEntry.setTime(timestamp);
            writeSignatureBlock(keySet, sfBytes, zipFileEntry.getOutputStream());
            zipOutput.write(zipFileEntry);
            if (canceled) return;
            ...
            if (canceled) {
                try {
                    ...
                    Log.w(TAG, t.getClass().getName() + ":" + t.getMessage());
                }
            }
        }
    }
